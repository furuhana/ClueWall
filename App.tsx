...
const App: React.FC = () => {
  ...
  // State for dragging/interaction
  ...
  // 🟢 智能轮询逻辑 (Smart Polling)
  useEffect(() => {
    let intervalId: NodeJS.Timeout;

    const loadData = async (isBackgroundRefresh = false) => {
      // 1. 如果用户正在操作，跳过刷新
      if (isBackgroundRefresh && isInteractingRef.current) return;

      // 2. 如果用户刚刚保存过 (3秒内)，跳过刷新
      // 这解决了“数据回弹”问题：防止服务器的旧缓存覆盖本地的新数据
      if (isBackgroundRefresh && (Date.now() - lastSaveTimeRef.current < 3000)) {
          // console.log("Just saved, skipping poll to prevent ghosting");
          return;
      }
      ...
      try {
        ...
        if (data && data.status === "success") {
           // 再次检查交互状态 (防止请求期间用户开始操作)
           if (!isInteractingRef.current) {
               
               // 🟢 3. 深度对比 (Deep Compare)
               // 只有当数据真的不同时，才调用 setNotes，避免无意义的 React 重渲染导致卡顿
               setNotes(prevNotes => {
                   // 简单暴力的 JSON 对比，对于这个数据量级足够快
                   const isSame = JSON.stringify(prevNotes) === JSON.stringify(data.notes);
                   return isSame ? prevNotes : data.notes;
               });

               setConnections(prevConns => {
                   const isSame = JSON.stringify(prevConns) === JSON.stringify(data.connections);
                   return isSame ? prevConns : data.connections;
               });
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
        if (isUIHiddenRef.current) setShowHiddenModeToast(true);
    }, 1000);
  ...
  const handleResizeStart = (e: React.MouseEvent, id: string, mode: ResizeMode) => {
      ...
      setTransformStart({
          mouseX: e.clientX, mouseY: e.clientY,
          initialRotation: note.rotation,
          initialWidth: dims.width, initialHeight: dims.height,
          initialX: note.x, initialY: note.y, initialScale: note.scale || 1,
          resizeMode: mode
      });
  };
  ...
  const handlePinMouseDown = (e: React.MouseEvent, id: string) => {
    ...
    setPinDragData({
        noteId: id, startX: e.clientX, startY: e.clientY,
        initialPinX: note.pinX ?? width / 2, initialPinY: note.pinY ?? 10,
        rotation: note.rotation, width, height
    });
  };
  ...
  const handleNodeMouseDown = (e: React.MouseEvent, id: string) => {
    ...
    if (e.altKey) {
         ...
         const duplicatedNote: Note = {
             ...targetNote, id: newId, zIndex: newZ,
             x: targetNote.x, y: targetNote.y, hasPin: false,
             title: targetNote.title ? `${targetNote.title} (Copy)` : undefined,
         };
         const nextNotes = [...notes, duplicatedNote];
         setNotes(nextNotes);
         setDraggingId(newId); setSelectedNodeId(newId);
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (resizingId && transformStart) {
        ...
        if (mode === 'CORNER') {
            ...
            setNotes(prev => prev.map(n => n.id === resizingId ? { 
             ...
             // Side resizing simplified for brevity
             ...
             if (mode === 'LEFT') { newWidth = Math.max(MIN_W, transformStart.initialWidth - localDx); newX = transformStart.initialX + localDx; }
             ...
             else if (mode === 'TOP') { newHeight = Math.max(MIN_H, transformStart.initialHeight - localDy); newY = transformStart.initialY + localDy; }
    ...
    const worldMouse = toWorld(e.clientX, e.clientY);
    if (connectingNodeId) setMousePos({ x: worldMouse.x, y: worldMouse.y });
    if (draggingId) {
  ...
  const handleZoomIn = () => {
      ...
      setView({ x: centerX - worldCenter.x * newZoom, y: centerY - worldCenter.y * newZoom, zoom: newZoom });
  };
  const handleZoomOut = () => {
  ...
  const handlePinClick = (e: React.MouseEvent, id: string) => {
    ...
    if (connectingNodeId === null) { setConnectingNodeId(id); } else {
      if (connectingNodeId !== id) {
        ...
        if (!exists) {
            ...
            setConnections(finalConns); saveToCloud(notes, finalConns);
        }
      }
      setConnectingNodeId(null);
    }
  };
  const handleDeleteConnection = (id: string) => { const nextConns = connections.filter(c => c.id !== id); setConnections(nextConns); saveToCloud(notes, nextConns); };
  ...
  const handleDeleteNote = (id: string) => {
    ...
    setNotes(nextNotes); setConnections(nextConns); setSelectedNodeId(null); saveToCloud(nextNotes, nextConns);
  };
  ...
  const addNote = (type: Note['type']) => {
     ...
     if (type === 'photo') content = 'New Evidence'; else if (type === 'scrap') content = 'Scrap note...'; else if (type === 'marker') { const existingMarkers = notes.filter(n => n.type === 'marker'); content = (existingMarkers.length + 1).toString(); }
     const newNote: Note = {
        id, type, content, title: type === 'dossier' ? 'TOP SECRET' : undefined, subtitle: type === 'dossier' ? 'CASE FILE' : undefined,
        x, y, zIndex: maxZIndex + 1, rotation: (Math.random() * 10) - 5, fileId: type === 'photo' ? '' : undefined, 
  ...
  const clearBoard = () => { if(window.confirm("Burn all evidence?")) { setNotes([]); setConnections([]); saveToCloud([], []); } };
  const handleDoubleClick = (id: string) => { if (!isPinMode && !connectingNodeId) setEditingNodeId(id); };
  const handleSaveNote = (updatedNote: Note) => { const nextNotes = notes.map(n => n.id === updatedNote.id ? updatedNote : n); setNotes(nextNotes); setEditingNodeId(null); saveToCloud(nextNotes, connections); };
  ...
  const handleDrop = useCallback(async (e: React.DragEvent) => {
    ...
    const dropX = worldPos.x; const dropY = worldPos.y;
    const promises = imageFiles.map(async (file, index) => {
        const driveFileId = await uploadImage(file); if (!driveFileId) return null;
        return new Promise<Note>((resolve) => {
            const img = new Image(); img.src = driveFileId;
            img.onload = () => {
                const MAX_WIDTH = 300; let finalWidth = img.width; let finalHeight = img.height;
                ...
                currentZ++;
                resolve({
                    id: `evidence-${Date.now()}-${index}`, type: 'evidence', content: file.name, fileId: driveFileId, 
                    x: dropX - (finalWidth / 2) + (index * 20), y: dropY - (finalHeight / 2) + (index * 20),
                    zIndex: currentZ, rotation: (Math.random() * 10) - 5, hasPin: false, width: finalWidth, height: finalHeight, scale: 1
                });
            };
        });
    });
    const loadedNotes = (await Promise.all(promises)).filter(n => n !== null) as Note[];
    if (loadedNotes.length > 0) { const newMaxZ = currentZ; setMaxZIndex(newMaxZ); const nextNotes = [...notes, ...loadedNotes]; setNotes(nextNotes); setSelectedNodeId(loadedNotes[loadedNotes.length - 1].id); saveToCloud(nextNotes, connections); }
  }, [maxZIndex, toWorld, notes, connections]);

  useEffect(() => { const globalUp = () => handleMouseUp(); window.addEventListener('mouseup', globalUp); return () => window.removeEventListener('mouseup', globalUp); }, [isPanning, draggingId, rotatingId, resizingId, pinDragData, notes, connections]);
  ...
  return (
    ...
    >
      ...
      {isLoading && <div className="absolute inset-0 z-[12000] flex items-center justify-center bg-black/50 backdrop-blur-sm text-white"><div className="flex flex-col items-center gap-4"><Loader2 className="animate-spin" size={48} /><span className="font-mono text-xl tracking-widest uppercase">Fetching Case Files...</span></div></div>}
      <div className="absolute top-6 left-0 w-full flex justify-center z-[11000] pointer-events-none"><div className="bg-black/70 backdrop-blur-md text-white/90 px-6 py-3 rounded-full border border-white/10 shadow-2xl flex items-center gap-3 transition-all duration-700 ease-in-out transform" style={{ opacity: showHiddenModeToast ? 1 : 0, transform: showHiddenModeToast ? 'translateY(0)' : 'translateY(-2rem)' }}><div className="w-2 h-2 rounded-full bg-red-500 animate-pulse shadow-[0_0_10px_rgba(239,68,68,0.5)]" /><span className="font-mono text-sm font-bold tracking-wide">PRESS ESC TO EXIT HIDDEN UI MODE</span></div></div>
