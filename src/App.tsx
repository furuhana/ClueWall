...
const App: React.FC = () => {
  ...
  useEffect(() => { interactionRef.current = { ...interactionRef.current, draggingId, resizingId, rotatingId, pinDragData, selectionBox, selectedIds, notes, connections, activeBoardId, isUIHidden, ghostNote, connectingNodeId, isPinMode }; }, [draggingId, resizingId, rotatingId, pinDragData, selectionBox, selectedIds, notes, connections, activeBoardId, isUIHidden, ghostNote, connectingNodeId, isPinMode]);
  ...
  useEffect(() => {
    ...
    // 🟢 补全：实时订阅逻辑
    const channel = supabase.channel(`realtime-board-${activeBoardId}`)
  ...
  const handleNodeMouseDown = (e: React.MouseEvent, id: string) => {
    ...
    if (isPinMode || (connectingNodeId && connectingNodeId !== id)) {
        ...
        if (connectingNodeId) {
            ...
            setNotes(prev => prev.map(n => n.id === id ? updated : n)); setConnections(prev => [...prev, newConn]); saveToCloud([updated], [newConn]); setConnectingNodeId(null);
        } else { setNotes(prev => prev.map(n => n.id === id ? updated : n)); saveToCloud([updated], []); }
        setIsPinMode(false); return;
    }
    if (e.altKey) {
        const target = notes.find(n => n.id === id);
        if (target) {
            const newId = `dup-${Date.now()}`; const newZ = maxZIndex + 1; setMaxZIndex(newZ);
            const newNode = { ...target, id: newId, x: target.x + 20, y: target.y + 20, zIndex: newZ, hasPin: false };
            setNotes(prev => [...prev, newNode]); saveToCloud([newNode], []); setDraggingId(newId); lastDragPosRef.current = { x: e.clientX, y: e.clientY };
            return;
        }
    }
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    if (selectionBox) {
        const currentX = e.clientX, currentY = e.clientY; setSelectionBox(prev => prev ? ({ ...prev, currentX, currentY }) : null);
    ...
    if (rotatingId && transformStart) { const deltaX = e.clientX - transformStart.mouseX; setNotes(prev => prev.map(n => n.id === rotatingId ? { ...n, rotation: transformStart.initialRotation - (deltaX * 0.5) } : n)); return; }
    if (isPanning && lastMousePosRef.current) { setView(v => ({ ...v, x: v.x + (e.clientX - lastMousePosRef.current!.x), y: v.y + (e.clientY - lastMousePosRef.current!.y) })); lastMousePosRef.current = { x: e.clientX, y: e.clientY }; return; }
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.ctrlKey && e.key.toLowerCase() === 'u') { e.preventDefault(); setIsUIHidden(prev => !prev); if (!isUIHidden) setShowHiddenModeToast(true); return; }
      if (interactionRef.current.ghostNote) {
          ...
          if (e.key === 'Enter') {
              const { x, y, typeIndex } = interactionRef.current.ghostNote;
              const type = NOTE_TYPES[typeIndex], L: any = { note:[256,160], photo:[256,280], dossier:[256,224], scrap:[257,50], marker:[30,30] }, dims = L[type] || [200,200];
              const newNode: Note = { id:`n-${Date.now()}`, type, content: type==='marker'?(interactionRef.current.notes.filter(n=>n.type==='marker').length+1).toString():'New Clue', x:x-dims[0]/2, y:y-dims[1]/2, zIndex:maxZIndex+1, rotation:0, hasPin:false, width:dims[0], height:dims[1], scale:1 };
              setNotes(prev=>[...prev, newNode]); saveToCloud([newNode], []); setGhostNote(null);
          }
      }
      if (e.key === 'Escape') { if (interactionRef.current.ghostNote) setGhostNote(null); else if (interactionRef.current.isUIHidden) setIsUIHidden(false); else { setConnectingNodeId(null); setIsPinMode(false); setSelectionBox(null); } }
      if (e.code === 'Space' && !e.repeat) setIsSpacePressed(true);
    };
    window.addEventListener('keydown', handleKeyDown); window.addEventListener('keyup', (e) => e.code === 'Space' && setIsSpacePressed(false));
  ...
  return (
    <div ref={boardRef} className={`w-screen h-screen relative overflow-hidden select-none ${isSpacePressed || isPanning ? 'cursor-grab active:cursor-grabbing' : ''}`} 
         style={{ backgroundColor: '#A38261', backgroundImage: `url("${GRID_URL}")`, backgroundPosition: `${view.x}px ${view.y}px`, backgroundSize: `${30 * view.zoom}px ${30 * view.zoom}px`, backgroundRepeat: 'repeat' }}
         onWheel={handleWheel} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp}
         onDrop={handleDrop} onDragOver={e => e.preventDefault()} onDragEnter={() => setIsDraggingFile(true)} onDragLeave={e => { dragCounter.current--; if(dragCounter.current<=0) setIsDraggingFile(false); }}
         onMouseDown={(e) => {
      ...
      {/* 补全：加载动画遮罩 */}
      {isLoading && <div className="absolute inset-0 bg-black/40 backdrop-blur-sm z-[20001] flex items-center justify-center flex-col gap-4">
          <Loader2 className="animate-spin text-yellow-500" size={48} />
          <span className="text-white font-mono tracking-widest animate-pulse">LOADING CASE DATA...</span>
      </div>}
        ...
        <div className="absolute top-4 left-4 z-[9999] flex flex-col gap-3 w-64" onMouseDown={e => e.stopPropagation()}>
          <div className="bg-black/80 backdrop-blur text-white p-4 rounded-lg border border-white/10 shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
                {['note','photo','dossier','scrap','marker'].map(t => <button key={t} onClick={() => setGhostNote({ ...toWorld(window.innerWidth/2, window.innerHeight/2), typeIndex:NOTE_TYPES.indexOf(t as any)})} className="px-2 py-1 bg-gray-700 hover:bg-gray-600 rounded text-[10px] font-bold uppercase transition-colors">{t}</button>)}
            </div>
      ...
      {/* 补全：文件拖拽上传提示层 */}
      {isDraggingFile && <div className="absolute inset-0 bg-black/60 z-[10000] flex items-center justify-center border-8 border-dashed border-gray-400 m-4 rounded-xl pointer-events-none animate-pulse"><div className="flex flex-col items-center gap-4 text-white"><UploadCloud size={64} className="text-blue-400"/><h2 className="text-2xl font-bold uppercase">Drop to Upload Evidence</h2></div></div>}
          ...
          {ghostNote && (() => {
              ...
              return (
                <div style={{ position:'absolute', left:ghostNote.x, top:ghostNote.y, transform:'translate(-50%, -50%)', zIndex:20000 }} className="pointer-events-none">
                    ...
                    <div className="mt-2 bg-black/50 text-white px-2 py-1 rounded text-[10px] font-mono text-center">SCROLL TO SWITCH • ENTER TO PLACE</div>
                </div>
              );
          })()}
