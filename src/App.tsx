...
const ClueWallApp: React.FC<ClueWallAppProps> = ({ session, userRole, onSignOut }) => {
    ...
    } = useBoardData(activeBoardId || undefined, interactionRef);
    ...
    const addNote = async (type: Note['type'], position?: { x: number, y: number }) => {
        ...
        const partialNote = {
            ...
            scale: 1, zIndex: maxZIndex + 1,
            rotation: (Math.random() * 10) - 5,
            file_id: type === 'photo' ? '/photo_1.png' : undefined,
            hasPin: false,
            board_id: Number(boardIdToUse),
    ...
    const presenceUser = session?.user?.id ? session.user.email?.split('@')[0] || userName : userName;

    // Pass View state explicitly for correct cursor projection
    const { otherUsers, broadcastMouse } = usePresence(
        activeBoardId,
        presenceUser,
    ...
    // 10. Event Processing Logic
    const handleMainMouseMove = (e: React.MouseEvent) => {
        // A. Broadcast Mouse Position (World Coordinates)
        const { x: worldX, y: worldY } = toWorld(e.clientX, e.clientY);
        broadcastMouse(worldX, worldY);
        setMousePos({ x: worldX, y: worldY });

        // B. Handle Interaction Logic
        // Prioritize Panning -> Pin Dragging -> Interaction Dragging (Notes)
        if (isPanning) {
    ...
    const handleWrapperDoubleClick = (id: number) => {
    ...
    return (
        ...
        >
            ...
            {/* Layer 2: Cursors */}
            {/* Must be above Transform Layer but below UI. Pointer-events-none essential. */}
            <CursorLayer otherUsers={otherUsers} view={view} />

            {/* Layer 3: Transform Canvas */}
            <div
            ...
            >
                ...
                {notes.map((note) => (
                    <DetectiveNode
                        ...
                        note={note}
                        onMouseDown={(e) => {
                            const wasPinClick = handleNodeClickForPin(e, note.id);
                            if (wasPinClick) return;
                            handleNodeMouseDown(e, note.id, isSpacePressed, isPinMode, connectingNodeId);
                        }}
                        ...
                        onDelete={() => { if (connectingNodeId === note.id) setConnectingNodeId(null); dataDeleteNote(note.id); }}
                ...
                <div className="absolute bottom-4 left-4 z-[12000] flex flex-col gap-2 pointer-events-none select-none font-sans">
                    ...
                    <div className="flex items-center gap-3 bg-black/40 backdrop-blur-sm px-3 py-1.5 rounded-full border border-white/10 transition-all">
                        ...
                        <div className="flex items-center">
                            {syncStatus === 'idle' && <span className="text-[10px] text-white/50 tracking-widest">安全连接就绪</span>}
                            {syncStatus === 'success' && (
                ...
                <>
                    ...
                    <div className="absolute top-4 right-4 z-[9999] flex flex-col gap-2 pointer-events-auto cursor-auto">
                        ...
                        <div className="bg-black/80 backdrop-blur text-white rounded-lg border border-white/10 flex flex-col items-center shadow-float">
                            ...
                            <button onClick={() => { setIsUIHidden(true); }} className="p-2 hover:bg-white/10 rounded-b-lg transition-colors" title="Hide UI"><Maximize size={20} /></button>
                        </div>
                    </div>
                </>
            )}
            ...
            {editingNodeId && notes.find(n => n.id === editingNodeId) && (
                <EditModal
                    ...
                    onSave={(note) => { updateNote(note); setEditingNodeId(null); }}
...
const App: React.FC = () => {
    ...
    const verifyProfileStrict = async (userId: string) => {
        ...
        try {
            ...
            if (!profile) {
                ...
                setSession(null);
                verifiedUserIds.current.delete(userId);
                return;
            }
            if (!profile.role) {
                return;
            ...
            verifiedUserIds.current.add(userId);
            setIsCheckingProfile(false);
        } catch (e) {
    ...
    useEffect(() => {
        supabase.auth.getSession().then(({ data: { session } }) => {
            ...
            if (session) verifyProfileStrict(session.user.id);
        });
    ...
    if (isLoadingAuth) return <div className="w-screen h-screen bg-black text-white flex items-center justify-center font-mono animate-pulse">Initializing Secure Connection...</div>;
    if (!session) return <Login loginMessage={loginMessage} />;
    if (isCheckingProfile) return <div className="w-screen h-screen bg-black text-white flex items-center justify-center font-mono flex-col gap-4"><Loader2 className="animate-spin text-yellow-500" size={48} /><div className="text-sm tracking-widest opacity-80 animate-pulse">Initializing Agent Profile...</div></div>;
